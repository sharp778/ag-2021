module.exports = [
  {
    title: "NLP",
    description: `Utilize NLP to understand the text, domain ontology to consolidate information from multiple and diverse sources`,
    icon: "/assets/images/solutions/hubs/nlp.svg",
  },
  {
    title: "Web Crawlers",
    description: `Using crawlers and NLP to search the internet for relevant publicly available data
      `,
    icon: "/assets/images/solutions/hubs/internet.png",
  },
  {
    title: `Insert Phrase Here`,
    description: `Enable alerts that mine global, local news as well as tweets for relevant insights`,
    icon: "/assets/images/solutions/hubs/internet.png",
  },
  {
    title: `Insert Phrase Here`,
    description: `Monitoring updates to websites/databases of interest and subscriptions`,
    icon: "/assets/images/solutions/hubs/internet.png",
  },
  {
    title: `Near Real Time Updates`,
    description: `In-built pull-based systems such as APIs for data sources where subscriptions are available, for near real-time updates`,
    icon: "/assets/images/solutions/hubs/time.svg",
  },
  {
    title: `Structured Information`,
    description: `Structuring the information in graph database enabling the flexibility to continuously expand data ingestion sources`,
    icon: "/assets/images/solutions/hubs/structure.svg",
  },
];
